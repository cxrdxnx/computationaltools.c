...
int main() 
{
    ...
    // insertar primeros m+1 elementos del array en el RBT. 
    int limite = m;
    if (limite > n - 1) limite = n - 1;
    ...
    for (int i = 0; i < n; i++) // por cada elemento del array.
    {    
        struct nodeRBTree* nodoMax = TreeMaximum(T);
        long long currentScore = (long long)nodoMax->key - arr[i];
        ...
        // eliminar el primer nodo en el arbol anterior.
        struct nodeRBTree* nodoEliminar = TreeSearch(T, arr[i]);
        if (nodoEliminar->key != NILKey) 
        { 
            T = RB_Delete(T, nodoEliminar);
        }
        
        // Insertar el siguiente nodo para mantener el rango.
        int nextIndex = i + m + 1;
        if (nextIndex < n) 
        {
            T = RB_Insert(T, arr[nextIndex]);
        }
    }
