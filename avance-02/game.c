...
int main()
{
    ...
    {
        int turnos = 0;
        struct nodeRBTree* EmmaList = AssignNilLeaf();
        struct nodeRBTree* OttoList = AssignNilLeaf();
        int n;
        scanf("%d", &n);
        for (int i = 0; i < n; i++)
        {
            int element;
            scanf("%d", &element);
            EmmaList = RB_Insert(EmmaList, element);
        }
        for (int i = 0; i < n; i++)
        {
            int element;
            scanf("%d", &element);
            OttoList = RB_Insert(OttoList, element);
        }

        while (EmmaList->key != NILKey && OttoList->key != NILKey)
        {
            struct nodeRBTree* OttoTurn = TreeMinimum(OttoList);
            struct nodeRBTree* EmmaTurn = TreeMaximum(EmmaList);
            int resultado = OttoTurn->key + EmmaTurn->key;
            OttoList = RB_Delete(OttoList, OttoTurn);
            EmmaList = RB_Delete(EmmaList, EmmaTurn);
            
            if (resultado % 2 != 0)
            {
                OttoList = RB_Insert(OttoList, resultado);
            }
            else
            {
                EmmaList = RB_Insert(EmmaList, resultado * 1/2);
            }

            turnos++;
        }

        if (OttoList->key == NILKey)
        {
            printf("%d %s\n", turnos, "Emma");
        }
        else
